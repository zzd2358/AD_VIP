...
const BASE_URL = "https://drive-m.quark.cn/1";
// 固定真实 Quark UA
const QUARK_UA =
...
const API = {
    info: [
        "/clouddrive/capacity/growth/v2/info",
        "/clouddrive/capacity/growth/info"
    ],
    sign: [
        "/clouddrive/capacity/growth/v2/sign",
        "/clouddrive/capacity/growth/sign"
    ],
    task: [
        "/clouddrive/capacity/growth/task"
    ]
};
...
async function doSign() {
    ...
    if (!infoData) {
        $.msg($.name, "❌ 获取签到信息失败", "Cookie 可能失效或网络问题");
        $.log("所有信息接口均尝试失败，可能是网络问题或 Cookie 失效。");
        return;
    }
    ...
    for (const path of API.sign) {
        try {
            ...
            if (data && (data.code === 0 || data.status === 200)) {
                ...
                $.log(`签到接口返回非预期数据: ${JSON.stringify(data)}`);
            }
        } catch (e) {
            $.log(`⚠️ 签到失败: ${e}`);
        }
    ...
    if (signSuccess) {
        ...
        $.msg($.name, "❌ 签到失败", "可能触发风控或网络异常，请稍后重试");
    }
}
